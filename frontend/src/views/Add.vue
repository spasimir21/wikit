...
import { AddTextAndRelationsRequest } from '../api/AddTextAndRelationsRequest';
...
const { loading, result, error, send, after } = useRequest(AddTextAndRelationsRequest);
...
afterTitle((error, result) => {
  if (error || !result || result.errors || !result.data.wikit) return;
...
<template>
  <div class="create-view">
    ...
    <div class="add-container" v-else>
      ...
      <button class="create-button" :disabled="loading" @click="addTextAndRelations">Add</button>
    </div>
  </div>
</template>
